...
class Commodity {
    ...
    public int getSoldPrice() {
        int tmp = (int) Math.round(1.02 * this.getTotal() / this.quantity);
        int res = ((tmp + 99) / 100) * 100;
        return res;
    }
...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Collections.sort(arr, new Comparator<Commodity>() {
            @Override
            public int compare(Commodity x, Commodity y) {
                return y.getSoldPrice() - x.getSoldPrice();
            }
        });
        for (Commodity x : arr)
